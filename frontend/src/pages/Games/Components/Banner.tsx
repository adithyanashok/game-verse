...
const Banner = ({
  ...
  return (
    <section className="relative overflow-hidden rounded-xl bg-[linear-gradient(135deg,#08101c_0%,#0d1424_48%,#08101c_100%)]">
      {imageUrl ? (
        <>
          <img
            src={imageUrl}
          ...
          <div className="absolute right-4 top-4 hidden w-[34%] max-w-90 overflow-hidden rounded-xl border border-white/10 bg-[#07101a]/50 shadow-[0_30px_90px_rgba(0,0,0,0.38)] backdrop-blur-md lg:block">
      ...
      <div className="relative z-10 min-h-80 px-5 py-6 sm:px-7 sm:py-8 lg:min-h-105 lg:px-8 lg:py-9">
        <div className="flex h-full max-w-195 flex-col justify-between gap-8 lg:pr-[34%]">
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-2 rounded-full border border-[rgba(0,212,255,0.16)] bg-[rgba(0,212,255,0.08)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-[#cbeafe]">
                <FiTag className="h-3.5 w-3.5 text-(--color-blue)" />
                {badgeLabel}
              </span>
              {game?.rating?.overallRating ? (
                <span className="rounded-full bg-(--color-lime) px-3 py-1 text-[11px] font-black uppercase tracking-[0.12em] text-[#07101a]">
                  {game.rating.overallRating.toFixed(1)} score
                </span>
              ) : null}
            </div>
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 text-sm font-semibold text-[#9aa7bd]">
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
                  <FiCalendar className="h-4 w-4 text-(--color-blue)" />
                  {formatReleaseDate(game?.releaseDate)}
                </span>
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
                  <FiTag className="h-4 w-4 text-(--color-blue)" />
      ...
      {!imageUrl ? (
        <div className="pointer-events-none absolute bottom-5 right-5 hidden rounded-[14px] border border-[rgba(0,212,255,0.12)] bg-[#07101a]/70 p-6 shadow-[0_30px_90px_rgba(0,0,0,0.34)] backdrop-blur-md lg:flex lg:w-62.5 lg:flex-col lg:items-center lg:justify-center lg:gap-3">
          <span className="flex h-14 w-14 items-center justify-center rounded-full border border-[rgba(0,212,255,0.18)] bg-[rgba(0,212,255,0.08)]">
            <FiImage className="h-6 w-6 text-(--color-blue)" />
          </span>
